...
async def overview():
    ...
    # Single aggregation pulling everything the overview needs.
    pipe = [
        {"$facet": {
            "active_diagnoses": [
                {"$match": {"status": "active"}},
    ...
    active_meds = await db.medications_master.find({"active": True}, {"_id": 0}).sort("last_seen", -1).limit(30).to_list(length=30)

    latest_vitals = {}
    for vtype in ["bp", "hr", "temp", "spo2", "weight", "bmi", "glucose"]:
        v = await db.vitals_timeline.find_one({"type": vtype}, sort=[("recorded_at", -1)])
        if v:
            v.pop("_id", None)
    ...
    red_flags = await db.reports.aggregate([
        ...
        {"$limit": 20},
        {"$unwind": "$red_flags"},
        {"$project": {"_id": 0, "report_id": 1, "uploaded_at": 1, "finding": "$red_flags.finding",
...
async def timeline(limit: int = 50):
    db = get_db()
    reports = await db.reports.find({}, {"_id": 0}).sort("uploaded_at", -1).limit(limit).to_list(length=limit)
    return {"reports": reports}


@router.get("/timeline/vitals/{vtype}")
async def vitals_series(vtype: str, limit: int = 30):
    db = get_db()
    rows = await db.vitals_timeline.find({"type": vtype}, {"_id": 0}).sort("recorded_at", 1).limit(limit).to_list(length=limit)
    return {"type": vtype, "points": rows}


@router.get("/timeline/labs/{test}")
async def labs_series(test: str, limit: int = 30):
    db = get_db()
    rows = await db.labs_timeline.find({"test": {"$regex": f"^{test}$", "$options": "i"}}, {"_id": 0}).sort("recorded_at", 1).limit(limit).to_list(length=limit)
...
async def get_report(report_id: str):
    ...
    suggestions = await db.suggestions.find({"report_id": report_id}, {"_id": 0}).to_list(length=100)
    consensus = await db.consensus_meta.find_one({"report_id": report_id}, {"_id": 0})
...
async def download_attachment(report_id: str, inline: bool = True):
    db = get_db()
    report = await db.reports.find_one({"report_id": report_id}, {"attachment_id": 1, "attachment_filename": 1, "attachment_mime": 1})
